...
export const ScreenMirror: React.FC<ScreenMirrorProps> = ({
    ...
    const { isSharing, startSharing, stopSharing } = useCaptureProvider(wsRef);
    ...
    return (
        ...
        >
            ...
            <div
                className={`absolute top-0 left-0 w-full h-1 z-20 ${status === 'connected' ? 'bg-success' : 'bg-error'
                    }`}
            />
            ...
            {!hasFrame && !isSharing && (
                <div className="text-neutral-500 text-center pointer-events-none z-10 px-6">
                    ...
                    {status === 'connected' && canShare && (
                        <button
                            ...
                            className="btn btn-primary btn-wide pointer-events-auto shadow-xl animate-in fade-in zoom-in duration-300"
                        >
                            Start Screen Share
                        </button>
                    )}
                    ...
                    {status === 'connected' && !canShare && (
                        <div className="bg-primary/10 text-primary text-xs p-4 rounded-2xl border border-primary/20 mb-6 max-w-xs mx-auto animate-in fade-in slide-in-from-bottom-2 duration-700 backdrop-blur-sm">
                            Waiting for Desktop stream...
                        </div>
                    )}

                    {!canShare && <div className="loading loading-ring loading-lg opacity-20 mt-4" />}
                </div>
            )}
