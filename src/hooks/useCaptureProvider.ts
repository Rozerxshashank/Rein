 ...
 */
export function useCaptureProvider(wsRef: React.RefObject<WebSocket | null>, status: 'connecting' | 'connected' | 'disconnected') {
    ...
    const captureFrame = useCallback(() => {
        ...
        if (!isSharingRef.current || !streamRef.current || !ws || isBusy.current || !video) return;
        ...
        try {
            ...
            canvas.toBlob((blob) => {
                ...
                if (!isSharingRef.current || !wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) return;
